...
def create_target(args):
    ...
    compose_bundles = {}
    try:
        # This is created by publish-compose-bundles if enabled.
        with open('/tmp/bundles.json') as f:
            compose_bundles = json.load(f)
    ...
    for tag, latest in latest_tags.items():
        for target in latest.values():
            ...
            if compose_bundles:
                target['custom']['docker_compose_bundles'] = compose_bundles
